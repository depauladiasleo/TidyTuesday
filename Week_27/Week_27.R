...
rent_summary |> 
  ...
  geom_density_ridges(aes(y = county, x = price, scale = 1.75), 
                      color = "#B71234", size = 0.85, fill = NA) +
  ggtitle(plot_title_text, subtitle = plot_subtitle_text) +
 ...
 #                             "marin", "contra costa", "alameda")) +
  scale_x_continuous(limits = c(0, 5000), expand = expansion(),
                     labels = scales::label_dollar()) +
  theme_minimal() +
  theme(
    plot.background = element_rect(color = "white"),
    plot.margin = margin(1, 1, 1, 1, unit = "cm"),
    plot.title = element_text(family = "Segoe UI", size = 20, color = "#B71234",
                              face = "bold", margin = margin(b = 2, unit = "pt")),
    plot.subtitle = element_text(family = "Segoe UI", size = 17, color = "gray60",
                                 margin = margin(b = 27, unit = "pt")),
    plot.title.position = "plot",
    plot.caption = element_text(family = "Segoe UI", size = 14,
                                color = "gray60", margin = margin(t = 14, unit = "pt")),
    panel.grid = element_blank(),
    axis.title = element_blank(),
    axis.text = element_text(family = "Segoe UI", size = 12, color = "gray60")) +
  labs(
